...
As we can see, the mean total number of steps taken per day is `r round(mean.steps, 4)`, and the median is `r round(median.steps, 4)`.
...
As we can see, the maximum number of steps on average across all the days is in the `r which.max(step.int$steps)`th interval, which is `r maxstep.int` minute.
...
As we can see, the mean total number of steps taken per day is `r round(mean.steps, 4)`, and the median is `r round(median.steps, 4)`. These values do differ from the estimates from the first part of the assignment. Because of the inputing missing data, the most frequent total number of steps increased. 
